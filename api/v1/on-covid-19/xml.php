...
try{
  ...
  if(is_array($object)){
    ...
    $data['timeToElpase'] = $time_to_elapse;
    $data['reportedCases'] = $reported_cases;
    $data['population'] = $population;
    $data['totalHospitalBeds'] = $total_hospital_beds;

    http_response_code(201);
    $xmlstr = "<region></region>";
    $sxe = new SimpleXMLElement($xmlstr);

    foreach (covid19ImpactEstimator($data) as $key => $value) {
      # code...
      ...
      if(is_array($value)){
        ...
        foreach ($value as $k => $val) {
          # code...
          $child->addChild($k, $val);
        }
      }else {
        $child = $region->addChild($key, $value);
      }
